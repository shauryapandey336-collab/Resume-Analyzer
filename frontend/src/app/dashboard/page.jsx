...
export default function Dashboard() {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    skills: [],
    job_match: 0,
    ai_suggestion: "No Suggestions Available",
  });
  ...
  const loadDashboard = useCallback(async (showLoader = false) => {
    try {
      ...
      console.log("Fresh Dashboard Response:", res.data);
      ...
      if (res.data?.success) {
        ...
        setDashboardData({
          ...
          ai_suggestion:
            Array.isArray(data.aiSuggestion?.suggestions)
              ? data.aiSuggestion.suggestions.join(", ")
              : data.aiSuggestion || "No Suggestions Available",
        });
      }
    } catch (err) {
  ...
  // Refresh whenever user comes back to this tab/page
  useEffect(() => {
      ...
      <section className="bg-gradient-to-r from-blue-700 to-indigo-700 text-white py-14">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <button
            ...
            disabled={refreshing}
            className="flex items-center gap-2 bg-white text-blue-700 px-4 py-2 rounded-xl font-semibold hover:bg-blue-50 disabled:opacity-50"
          >
            <RefreshCw
              size={18}
